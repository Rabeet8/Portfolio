...
export default function RadialOrbitalTimeline({
  ...
  const rotationAngle = useMotionValue(230);
  ...
  const toggleItem = (id: number) => {
    ...
    setExpandedItems((prev) => {
      ...
      if (isAboutToExpand) {
        ...
        // Calculate target rotation to bring item to the top (270 degrees)
        const targetBase = 270 - (index / totalItems) * 360;
        
        // Find shortest path for rotation
        const current = rotationAngle.get();
        const diff = (targetBase - current) % 360;
        const shortestDiff = ((diff + 540) % 360) - 180;
        const finalTarget = current + shortestDiff;
        
        animate(rotationAngle, finalTarget, {
          type: "spring",
          stiffness: 60,
          damping: 20,
        });
    ...
    const rotate = (time: number) => {
      if (autoRotate) {
        if (lastTime !== 0) {
          ...
          rotationAngle.set(rotationAngle.get() + delta * 0.015);
        }
        lastTime = time;
...
function OrbitalNode({
  ...
  const x = useTransform(rotationAngle, (val: number) => {
    const angle = (index / total) * 360 + val;
    return 240 * Math.cos((angle * Math.PI) / 180);
  });

  const y = useTransform(rotationAngle, (val: number) => {
    const angle = (index / total) * 360 + val;
    return 240 * Math.sin((angle * Math.PI) / 180);
  });
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.relatedIds.map((relatedId) => {
                        ...
                        return (
                          ...
                          >
                            {relatedItem?.title || `Phase ${relatedId}`}
